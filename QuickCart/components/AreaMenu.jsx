...
const AreaMenu = ({ className = "", horizontal = false }) => {
  const searchParams = useSearchParams();
  const router = useRouter();
  const active = (searchParams.get("q") || "").toLowerCase();

  const areas = [
    { name: "Muebles", value: "Muebles" },
    { name: "Electrodomésticos", value: "Electrodomésticos" },
    { name: "Movilidad", value: "Movilidad" },
  ];

  const goToArea = (value) => {
    router.push(`/all-products?q=${encodeURIComponent(value)}`);
  };
  ...
  return (
    <ul className={ulBase}>
      {areas.map((a) => {
        const isActive = active && a.value.toLowerCase() === active;
        return (
          <li key={a.value}>
            <button onClick={() => goToArea(a.value)} className={itemClass(isActive)}>
